from sqlalchemy import Column, VARCHAR, ForeignKey, DATETIME, INTEGER, TEXT
...
class Society(db.Model):
    ...
    def insert(s_name):
        ...
        with db.auto_commit():
            society = Society()
            society.id = create_id(s_name)
            society.name = s_name
            society.update_time = time
            society.create_time = time
...
class Section(db.Model):
    ...
    def insert(o, name):
        ...
        with db.auto_commit():
            section = Section()
            section.id = create_id(o.name+name)
            section.name = name
...
class Apply(db.Model):
    ...
    first_section_id = Column(VARCHAR(64), ForeignKey('section.id'), nullable=False, comment='意向部门编号')
    second_section_id = Column('Second Section', VARCHAR(64), comment='第二意向部门编号')
    adjust = Column('Adjust', VARCHAR(9), comment='是否服从调剂')
    # ------------------------------------------------------------------
    skill = Column('Skill', TEXT, comment='特长技能')
    ...
    @staticmethod
    def insert_to_db(forms, o):
        """ 向数据库写入用户信息 """
        time = get_time()
        with db.auto_commit():
            user = Apply()
            user = process_data(user, o, forms, time)
            db.session.add(user)

    @staticmethod
    def update_user_info(forms, o):
        """ 更新数据库用户信息 """
        with db.auto_commit():
            user_info = Apply.query.filter_by(student_id=forms.pop('student_id')).first()
            process_data(user_info, o, forms, get_time())
...
def process_data(user, o, forms, time):
    ...
    user.first_section_id = forms.get('first_section_id')
